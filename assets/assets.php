...
function mm_load_scripts_and_libs()
{
	...
	// Load Boxicons.
	wp_enqueue_script('boxicons-js', get_template_directory_uri() . '/assets/js/boxicons.min.js', array(), THEME_VERSION, true);

	if (is_home()) {
		// call flickity js.
		wp_enqueue_script('flickity-js', get_template_directory_uri() . '/assets/js/flickity.min.js', array(), THEME_VERSION, true);

		// Call Infinite Scroll.
		wp_enqueue_script('infinite-js', get_template_directory_uri() . '/assets/js/infinite-scroll.min.js', array(), THEME_VERSION, true);
	...
	if (is_single()) {
		...
		$has_code = carbon_get_post_meta($the_post_id, 'bb_post_code_highlighter');
		if ($has_code) {

			// Call Prism.css.
			wp_enqueue_style('prism-css', THEME_URI . '/assets/libs/prism.css', array(), '1.25.0');


			// Call Prism.js.
			wp_enqueue_script('prism-js', THEME_URI . '/assets/libs/prism.min.js', array(), '1.25.0', null, true);
		}
